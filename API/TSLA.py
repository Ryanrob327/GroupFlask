...
TSLA_api = Blueprint('TSLA_api', __name__,
                   url_prefix='/api/TSLA')
...
class JokesAPI:
    ...
    class _Read(Resource):
        def get(self):
            return jsonify(getJokes(TSLA_data))

    # getJoke(id)
    class _ReadID(Resource):
        def get(self, id):
            return jsonify(getJoke(TSLA_data, id))

    # getRandomJoke()
    class _ReadRandom(Resource):
        def get(self):
            return jsonify(getRandomJoke(TSLA_data))
    
    # getRandomJoke()
    class _ReadCount(Resource):
        def get(self):
            count = countJokes(TSLA_data)
